<div class="py-10 bg-slate-50 border-b border-slate-200">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Hero Header -->
        <div class="bg-gradient-to-r from-brand-dark via-brand-navy to-brand-blue p-6 sm:p-8 rounded-3xl text-white shadow-xl relative overflow-hidden mb-8">
            <div class="flex items-center gap-2 mb-3">
                <span class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-blue-400/20 text-blue-200 uppercase tracking-wider border border-blue-400/30">
                    <?php echo $service['category_name']; ?>
                </span>
                <?php if (!empty($service['badge'])): ?>
                    <span class="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-emerald-400/20 text-emerald-300 border border-emerald-400/30">
            ...
            <div class="flex flex-wrap items-center gap-6 text-xs sm:text-sm text-slate-200 pt-4 border-t border-white/10">
                ...
                <div class="text-lg font-black text-white">
                    ...
                    <span class="text-xs font-normal text-blue-200"><?php echo $service['govt_fee_note']; ?></span>
                </div>
            </div>
        </div>
                    ...
                    <div class="space-y-2.5 text-xs sm:text-sm text-slate-700">
                        <?php foreach ($service['deliverables'] as $deliv): ?>
                            <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 border border-slate-100">
                                ...
                                <span class="font-medium"><?php echo $deliv; ?></span>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
                    ...
                    <div class="space-y-2 text-xs sm:text-sm text-slate-700">
                        <?php foreach ($service['documents_required'] as $doc): ?>
                            <div class="flex items-center gap-2.5 p-3 rounded-xl border border-slate-200 bg-white">
                                <i data-lucide="file-check" class="w-4 h-4 text-brand-blue flex-shrink-0"></i>
                                <span class="font-medium"><?php echo $doc; ?></span>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <div id="apply" class="lg:col-span-5 sticky top-24">
                <div class="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200 shadow-2xl space-y-5">
                    <div class="border-b border-slate-100 pb-4">
                        <span class="text-xs font-bold uppercase tracking-wider text-emerald-800 bg-emerald-100 px-2.5 py-1 rounded-full">
                            Online Application
                        </span>
                    ...
                    <form id="service-order-form" onsubmit="handleOrderSubmit(event)" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="customer_name" required placeholder="As per PAN Card" class="w-full h-11 px-3.5 rounded-xl border border-slate-200 text-sm focus:border-brand-blue outline-none">
                        </div>

                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 mb-1">Mobile (+91) *</label>
                                <input type="tel" name="phone" required placeholder="98765 43210" class="w-full h-11 px-3.5 rounded-xl border border-slate-200 text-sm focus:border-brand-blue outline-none">
                            </div>
                            <div>
                                <label class="block text-xs font-semibold text-slate-700 mb-1">Email *</label>
                                <input type="email" name="email" required placeholder="you@domain.com" class="w-full h-11 px-3.5 rounded-xl border border-slate-200 text-sm focus:border-brand-blue outline-none">
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Proposed Company / Brand Name</label>
                            <input type="text" name="entity_name" placeholder="e.g. TaxsPoint Tech Pvt Ltd" class="w-full h-11 px-3.5 rounded-xl border border-slate-200 text-sm focus:border-brand-blue outline-none">
                        </div>
                        ...
                        <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-2">
                            <div class="flex justify-between">
                                ...
                                <strong class="text-slate-900">₹<?php echo number_format($service['price']); ?></strong>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-slate-500">GST (18%):</span>
                                <span class="text-slate-700">₹<?php echo number_format(round($service['price'] * 0.18)); ?></span>
                            </div>
                            <div class="flex justify-between pt-2 border-t border-slate-200 text-sm font-bold text-brand-blue">
                                <span>Total Amount:</span>
                                <span>₹<?php echo number_format(round($service['price'] * 1.18)); ?></span>
                            </div>
                        </div>

                        <button type="submit" class="w-full py-3.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-bold text-sm shadow-lg shadow-emerald-600/20 transition flex items-center justify-center gap-2">
                            <i data-lucide="lock" class="w-4 h-4"></i>
                            <span>Submit Application & Pay</span>
                        </button>
                    </form>

                    <div id="order-success-msg" class="hidden py-6 text-center space-y-3 bg-emerald-50 rounded-2xl border border-emerald-200 p-6">
                        ...
                        <h4 class="text-lg font-bold text-slate-900">Application Submitted!</h4>
                        <p class="text-xs text-slate-600">Your application has been received. Order ID generated.</p>
                        <a id="track-order-btn" href="<?php echo site_url('track'); ?>" class="inline-block mt-3 px-5 py-2.5 rounded-xl bg-brand-navy hover:bg-brand-blue text-white font-bold text-xs">
